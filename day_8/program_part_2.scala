...
//  var i = 0
//  for ((distance, pairs) <- distanceMap) {
//    for ((junctionBox1, junctionBox2) <- pairs) {
//      i+=1
//      println(s"$i Distance: $distance between $junctionBox1 and $junctionBox2")
//    }
//  }

  var realCount = 0
  var lastJunctionBoxes: Tuple2[JunctionBox, JunctionBox] = null
  for((shortestDistance, i) <- distanceMap.zipWithIndex if (circuits.count(s => s.size == junctionBoxes.size) != 1)) {
      //Add a circuit between the two junction boxes if junctionboxes are not already in circuit
    for((junctionBox1, junctionBox2) <- shortestDistance._2 if (circuits.count(s => s.size == junctionBoxes.size) != 1)) {
      realCount += 1
      ...
      lastJunctionBoxes = (junctionBox1, junctionBox2)
    ...
    println(s"Step $realCount cc: ${circuits.count(s => s.size > 0)}")
...
//  }
  ...
  println(s"Last Junction Boxes: ${lastJunctionBoxes._1} and ${lastJunctionBoxes._2}")
  println(s"Answer: ${lastJunctionBoxes._1.x * lastJunctionBoxes._2.x}")
}
